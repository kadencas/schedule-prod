...
export async function POST(request: Request) {
  try {
    ...
    for (const shift of payload) {
      ...
      const startDateTime = new Date(`${shiftDateStr}T${shift.startTime}:00`);
      const endDateTime   = new Date(`${shiftDateStr}T${shift.endTime}:00`);
      ...
      for (const seg of segments) {
        if (!seg.id) continue;
        const segStart = new Date(`${shiftDateStr}T${seg.startTime}:00`);
        const segEnd   = new Date(`${shiftDateStr}T${seg.endTime}:00`);
