...
const mergeSegments = (segments) => {
  ...
  const merged = [JSON.parse(JSON.stringify(sortedSegments[0]))];
  ...
  return (
    <div className="mb-3 flex justify-between items-center">
      ...
      <div className="flex items-center space-x-1">
        ...
        <button onClick={onToggleCalendar} className="p-1.5 border rounded-md text-gray-500 hover:bg-gray-100" title="Toggle Calendar">
...
export default function App() {
  ...
  // moved out so we can call from the Refresh button
        ...
        <div className="bg-white p-3 rounded-xl shadow-sm">
          <div className="mb-3">
            <input
              ...
              className="w-full px-3 py-1.5 border border-gray-300 rounded-md focus:outline-none focus:ring"
            />
          </div>

          <div className="grid grid-cols-9 gap-2 mb-3 border-b pb-2">
            <div className="col-span-2 font-semibold text-gray-600 text-sm">Entity</div>
            {weekDays.map(day => (
              <div key={day} className="text-center font-semibold text-gray-600 col-span-1 text-sm">{day}</div>
            ))}
          </div>
            ...
            <div className="space-y-2">
              {filteredEntities.length > 0 ? (
                filteredEntities.map(entity => (
                  <div key={entity.id} className="grid grid-cols-9 gap-2 items-center">
                ...
                <div className="text-center text-gray-500 py-8">No entities match your search.</div>
              )}
            </div>
          )}
        </div>
